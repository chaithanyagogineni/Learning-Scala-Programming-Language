//Problem 11 in Project Euler
//Implemented in scala


//Function that returns the maximum products among the up,down,left,right,diagonal of number
def getProduct(value:Int,row:Int,col:Int,input:Array[Array[Int]]):BigInt ={
  var up:BigInt=1
  var down:BigInt=1
  var left:BigInt=1
  var right:BigInt =1
  var downdiagonal:BigInt =1
  var updiagonal:BigInt = 1
  //compute the Up product
  if(row>=3)
  {
    for(i<-row-3 to row)
    {
  ...
  //Compute the down diagonal product
  if(row<=16&&col<=16)
  {
    var dstart=row
    var dend=col
    for(i<- 1 to 4)
    {
      downdiagonal*=input(dstart)(dend)
      dstart+=1
      dend+=1
    }
  }
  //compute the up diagonal product
  if(row>=3&&col<=16)
  {
    var s=row
    var e=col
